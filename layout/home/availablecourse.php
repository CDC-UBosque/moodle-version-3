 ...
 * 
 */
$course = $DB->get_records_sql('SELECT c.* FROM {course} c where id != ? AND visible = ?', array(0, 0));  
...
foreach ($course as $key => $coursevalue) {
    ...
    if ( !empty( $courseimage ) ) {
        ...
        $coursedetailsarray[$key]["courseimage"] = $CFG->wwwroot."/theme/uebtheme/data/nopic.jpg";
    }
    $courseimage = '';
}
?>
