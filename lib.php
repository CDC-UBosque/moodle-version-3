...
/**
 * theme_uebtheme lib file.
 * @package    theme_uebtheme
 * @copyright  2015 dualcube {@link https://dualcube.com}
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
/**
 * theme_uebtheme_get_setting function for load custom settings.
 *
 * @param string $setting
 * @param string $format
 * @return $theme->settings->$setting
 */
function theme_uebtheme_get_setting($setting, $format = false) {
    global $CFG;
    require_once($CFG->dirroot . '/lib/weblib.php');
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('uebtheme');
    }
 ...
 */
function theme_uebtheme_process_css($css, $theme) {

    // Set the background image for the logo.
    $logo = $theme->setting_file_url('logo', 'logo');
    $css = theme_uebtheme_set_logo($css, $logo);
    if (!empty($theme->settings->fontnamebody)) {
        $font = $theme->settings->fontnamebody;
    } else {
        $font = 'Raleway';
    }
    $headingfont = theme_uebtheme_get_setting('fontnameheading');
    $bodyfont = theme_uebtheme_get_setting('fontnamebody');

    $css = theme_uebtheme_set_headingfont($css, $headingfont);
    $css = theme_uebtheme_set_bodyfont($css, $bodyfont);
    $css = theme_uebtheme_set_fontfiles($css, 'heading', $headingfont);
    $css = theme_uebtheme_set_fontfiles($css, 'body', $bodyfont);
    // Set custom CSS.
    ...
    $css = theme_uebtheme_set_customcss($css, $customcss);
    $thememaincolor = theme_uebtheme_get_setting('maincolor');
    $css = theme_uebtheme_set_maincolor($css, $thememaincolor);
    return $css;
}
/**
 ...
 */
function theme_uebtheme_set_maincolor($css, $themecolor) {
 ...
 * @return string The CSS which now contains our custom CSS.
 */
function theme_uebtheme_set_headingfont($css, $headingfont) {
 ...
 * @return string The CSS which now contains our custom CSS.
 */
function theme_uebtheme_set_bodyfont($css, $bodyfont) {
 ...
 */
function theme_uebtheme_set_logo($css, $logo) {
    GLOBAL $CFG;
    $tag = '[[setting:logo]]';
    $replacement = $logo;
    if (is_null($replacement)) {
        $replacement = $CFG->wwwroot.'/theme/uebtheme/css/img/logo.png';
    }
 ...
 * @return string The parsed CSS
 */
function theme_uebtheme_set_fontfiles($css, $type, $fontname) {
    $tag = '[[setting:fontfiles' . $type . ']]';
    $replacement = '';
    if (theme_uebtheme_get_setting('fontselect') === '2') {
        static $theme;
        if (empty($theme)) {
            $theme = theme_config::load('uebtheme');
        }
 ...
 * @return bool
 */
function theme_uebtheme_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;
    if (empty($theme)) {
        $theme = theme_config::load('uebtheme');
    }
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        ...
        } else if ($filearea === 'masonryimage9') {
            return $theme->setting_file_serve('masonryimage9', $args, $forcedownload, $options);
        ...
        } else if ($filearea === 'banneruebimage1') {
            return $theme->setting_file_serve('banneruebimage1', $args, $forcedownload, $options);
        } else if ($filearea === 'banneruebimage2') {
            return $theme->setting_file_serve('banneruebimage2', $args, $forcedownload, $options);
        } else {
            send_file_not_found();
        }
    } else {
 ...
 * @return string The CSS which now contains our custom CSS.
 */
function theme_uebtheme_set_customcss($css, $customcss) {
 ...
 *
 * Do not add uebtheme specific logic in here, child themes should be able to
 ...
 */
function theme_uebtheme_get_html_for_settings(renderer_base $output, moodle_page $page) {
    global $CFG, $USER;
    $return = new stdClass;
    ...
    /*-----------------------for banners*--------------------------*/
    if (!empty($page->theme->settings->banner1)) {
        $return->banner1 = $page->theme->settings->banner1;
    }
    if (!empty($page->theme->settings->bannerueburl1)) {
        $return->bannerueburl1 = $page->theme->settings->bannerueburl1;
    }
    if (!empty($page->theme->settings->banner2)) {
        $return->banner2 = $page->theme->settings->banner2;
    }
    if (!empty($page->theme->settings->bannerueburl2)) {
        $return->bannerueburl2 = $page->theme->settings->bannerueburl2;
    }
    /*-----------------------for social contact*--------------------------*/
...
/**
 * All theme functions should start with theme_uebtheme_
 * @deprecated since 2.5.1
 */
function uebtheme_process_css() {
 ...
 * @deprecated since 2.5.1
 */
function uebtheme_set_customcss() {
    throw new coding_exception('Please call theme_'.__FUNCTION__.' instead of '.__FUNCTION__);
}
